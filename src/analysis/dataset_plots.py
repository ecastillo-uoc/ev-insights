"""
EDA / thesis-chapter dataset visualisations.

All ``fetch_and_plot_*`` functions combine a data fetch (from
``src.data.data_fetcher``) with a corresponding ``plot_*`` helper.
The ``plot_*`` functions accept a DataFrame directly, so they can be
reused independently of the database layer.

Migrated from the former ``src.tfm.tfm_plots`` module.
"""
...
from src.data.constants import COVID_START, COVID_END
...
def fetch_and_plot_dataset_trends(dataset_name: str, db_config: dict, fig_dir: Path, site_name: str = None):
    ...
    if not dataset.empty:
        ...
        logger.warning("No data found for the %s dataset.", dataset_name)


def plot_energy_consumption_trends_by_site(dataset: pd.DataFrame, time_col: str = 'timestamp', energy_col: str = 'energy_kwh',
                                           site_col: str = 'site', fig_dir: Path = None, dataset_name: str = None):
...
def fetch_and_plot_energy_trends_by_site(dataset_name: str, db_config: dict, fig_dir: Path):
    ...
    if not dataset.empty:
        plot_energy_consumption_trends_by_site(dataset, fig_dir=fig_dir, dataset_name=dataset_name)
    else:
        logger.warning("No data found for the %s dataset.", dataset_name)


# ── Charges / sessions trends ──────────────────────────────────────────────


def plot_charges_trends(dataset: pd.DataFrame, time_col: str = 'timestamp', count_col: str = 'sessions_count',
                        fig_dir: Path = None, dataset_name: str = None):
...
def fetch_and_plot_dataset_charges_trends(dataset_name: str, db_config: dict, fig_dir: Path, site_name: str = None):
    ...
    if not dataset.empty:
        plot_charges_trends(dataset, fig_dir=fig_dir, dataset_name=dataset_name)
    else:
        logger.warning("No data found for the %s dataset.", dataset_name)


def plot_charges_trends_by_site(dataset: pd.DataFrame, time_col: str = 'timestamp', count_col: str = 'sessions_count',
                                site_col: str = 'site', fig_dir: Path = None, dataset_name: str = None):
...
def fetch_and_plot_charges_trends_by_site(dataset_name: str, db_config: dict, fig_dir: Path):
    ...
    if not dataset.empty:
        plot_charges_trends_by_site(dataset, fig_dir=fig_dir, dataset_name=dataset_name)
    else:
        logger.warning("No data found for the %s dataset.", dataset_name)
...
def fetch_and_plot_multiple_datasets_trends(dataset_names: list, db_config: dict, fig_dir: Path, plot_name: str = 'combined'):
    ...
    for dataset_name in dataset_names:
        ...
        if not df.empty:
            ...
            logger.warning("No data found for the %s dataset.", dataset_name)
    if datasets:
        plot_multiple_energy_consumption_trends(datasets, fig_dir=fig_dir, plot_name=plot_name)
...
def plot_charges_by_month(dataset: pd.DataFrame, dataset_name: str, fig_dir: Path):
    """Plots the number of charges by month."""
...
def fetch_and_plot_charges_by_month(dataset_name: str, db_config: dict, fig_dir: Path, site_name: str = None):
    """Fetches the dataset and plots the total number of charges by month."""
    dataset = fetch_dataset_plug_ins(dataset_name, db_config)
    if not dataset.empty:
...
def tag_covid_period(dataset: pd.DataFrame) -> pd.DataFrame:
    """Tags the dataset with 'Pre-COVID' and 'Post-COVID' periods."""
    dataset['plug_in_datetime'] = pd.to_datetime(dataset['plug_in_datetime'])
...
def plot_charges_by_weekday_covid(dataset: pd.DataFrame, dataset_name: str, fig_dir: Path):
    ...
    grouped_pct.T.plot(kind='bar', figsize=(12, 6), alpha=0.8, edgecolor='black', width=0.7)
    ...
    # Absolute plot
    grouped.T.plot(kind='bar', figsize=(12, 6), alpha=0.8, edgecolor='black', width=0.7)
...
def fetch_and_plot_time_serie_decomposition(dataset_name: str, db_config: dict, fig_dir: Path, site_name: str = None):
    """Fetches the energy trends and plots additive time-series decomposition."""
    dataset = fetch_dataset_energy_trends(dataset_name, db_config)
    if dataset.empty:
        logger.warning("No data for %s", dataset_name)
        return
    ...
    time_serie_decomposition = sm.tsa.seasonal_decompose(
        x=dataset['energy_kwh'].dropna(), model='additive', period=7,
    )

    fig, axes = plt.subplots(4, 1, figsize=(14, 12), sharex=True)

    axes[0].plot(time_serie_decomposition.observed, label='Observed', color='blue')
    axes[0].set_ylabel('Observed'); axes[0].legend(loc='upper left')
    ...
    axes[1].set_ylabel('Trend'); axes[1].legend(loc='upper left')
    axes[1].grid(True, linestyle='--', alpha=0.3)

    axes[2].plot(time_serie_decomposition.seasonal, label='Seasonal', color='green')
    axes[2].set_ylabel('Seasonal'); axes[2].legend(loc='upper left')
    ...
    axes[3].set_ylabel('Residual'); axes[3].set_xlabel('Time')
    axes[3].legend(loc='upper left'); axes[3].grid(True, linestyle='--', alpha=0.3)
...
def fetch_and_plot_charges_time_serie_decomposition(dataset_name: str, db_config: dict, fig_dir: Path, site_name: str = None):
    """Fetches the charges trends and plots additive time-series decomposition."""
    dataset = fetch_dataset_charges_trends(dataset_name, db_config)
    if dataset.empty:
        logger.warning("No data for %s", dataset_name)
        return
    ...
    time_serie_decomposition = sm.tsa.seasonal_decompose(
        x=dataset['sessions_count'].dropna(), model='additive', period=7,
    )
...
def fetch_and_plot_session_boxplots_by_site(dataset_name: str, db_config: dict, fig_dir: Path):
    """Fetches session details from DB and generates energy and duration boxplots."""
    ...
    if not dataset.empty:
        ...
        logger.warning("No session details found for the %s dataset.", dataset_name)


def fetch_and_plot_session_boxplots_by_specific_site(dataset_name: str, site_name: str, db_config: dict, fig_dir: Path):
    """Fetches session details by a specific site then generates boxplots."""
    ...
    if not dataset.empty:
        ...
        logger.warning("No session details found for %s at site %s.", dataset_name, site_name)
...
def fetch_and_plot_session_boxplots_by_station(station_ids: tuple = None, city: str = None, db_config: dict = None, fig_dir: Path = None):
    """Fetches session details by station IDs and/or city and generates boxplots."""
    ...
    if not dataset.empty:
        ...
        logger.warning("No session details found for station IDs %s and city %s.", station_ids, city)
