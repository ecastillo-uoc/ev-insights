...
class TransformerModelStrategy(ModelStrategy):
    ...
    def build_model(self, input_shape, head_size=32, num_heads=2, ff_dim=32, num_transformer_blocks=1, mlp_units=[64], dropout=0.1, mlp_dropout=0.1, learning_rate=0.001):
        """
        Builds a Transformer time-series forecasting model.
        """
        ...
        for _ in range(num_transformer_blocks):
            ...
            # Feed Forward Part
            x2 = LayerNormalization(epsilon=1e-6)(attention_output)
            x3 = Dense(ff_dim, activation="relu")(x2)
            x3 = Dropout(dropout)(x3)
            x3 = Dense(inputs.shape[-1])(x3)
            x = Add()([x3, attention_output])
    ...
    def train(self,
        ...
        try:
            # For Transformer, we look for custom parameters or use defaults
