...
class xgboost_charge_duration(Forecast):
        ...
        for feature, filters in self.custom_params.items():
            ...
            if feature == 'plug_in_hour_minutes':
                ...
                if 'plug_in_hour_minutes' not in self.df.columns:
                    self.df['plug_in_hour_minutes'] = self.df['plug_in_datetime'].apply(lambda row: utc_to_decimal_hours_minutes(row))
    ...
    def train(self):
        ...
        for dataset_name in self.datasets_names:
            ...
            # remove target
            target = np.array(df['plug_duration'])
            df = df.drop('plug_duration', axis=1)
            ...
            self.logger.info('Results for plug_duration')
    ...
    def predict(self):
        ...
        for dataset_name in self.datasets_names:
            if self.submode == 'schedule':
                ...
                est_duration = self.model.predict(features)

                output_dict['predict'].update(
                    {
                        'duration': round(est_duration[0]),
                        'date': self.date,
