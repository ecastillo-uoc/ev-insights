...
class PostgreSql(Interface):
    ...
    def save_forecast_model(self, forecaster_name, results, file_path, algo=None):
        """Not implemented: models are stored via MLflow or filesystem (model_persistence), not in evinsights DB."""
        raise NotImplementedError("PostgreSQL interface does not store models. Use MLflow or model_persistence.")

    def get_model(self, algo, model_name, models_dir=None):
        """Not implemented: models are loaded via MLflow or filesystem (model_persistence), not from evinsights DB."""
        raise NotImplementedError("PostgreSQL interface does not store models. Use MLflow or model_persistence.")
